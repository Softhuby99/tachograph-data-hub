...
// AUTH_MODE is a deployment-level flag (set once per image), not a per-request
// secret, so reading it at module scope is safe on both Node (Docker) and
// Cloudflare Workers (where it is undefined -> requireSupabaseAuth).
